...
Anyone can *claim* their tool works offline. The `no-egress` CI job replaces
`socket()` with something that raises, runs the entire pipeline, and fails the
build if anything attempts a connection. The green badge means *this binary
cannot phone home* — checked on every commit.
...
print(analyse("corpus/reference1.sigmf-meta").summary())
PY
```
...
- [x] `LICENSE` (Apache-2.0) + `NOTICE` with the full dependency position
- [x] `Dockerfile` — multi-stage, non-root, healthcheck
- [x] CI: tests · **no-egress** · SBOM · licence audit · frontend · docker
- [x] `cartridges/` under CC-BY-4.0, separate from the code
- [x] `tests/` — unit plus the round-trip oracle
